...
_start:
    ...
    lui      t0, %hi(step)                   ; Load upper immediate of 'step' into t0
    addi     t0, t0, %lo(step)               ; Add lower immediate part of 'step' to t0
    lw       t1, 0(t0)                       ; Load the value of 'step' into t1 (t1 = step)

    ; Load the accumulator value into register t2
    lui      t0, %hi(acc)                    ; Load upper immediate of 'acc' into t0
    addi     t0, t0, %lo(acc)                ; Add lower immediate part of 'acc' to t0
    lw       t2, 0(t0)                       ; Load the value of 'acc' into t2 (t2 = acc)

    ; Load the address of the output port into register t4
    lui      t3, %hi(out_port)               ; Load upper immediate of 'out_port' into t3
    addi     t3, t3, %lo(out_port)           ; Add lower immediate part of 'out_port' to t3
    lw       t4, 0(t3)                       ; Load the value of 'out_port' into t4 (t4 = out_port)

    ; Load the limit value into register t5
    lui      t6, %hi(limit)                  ; Load upper immediate of 'limit' into t6
    addi     t6, t6, %lo(limit)              ; Add lower immediate part of 'limit' to t6
