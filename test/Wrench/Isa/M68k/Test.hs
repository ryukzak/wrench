...
tests =
    testGroup
        ...
        [ testCase "movea.l D0, A0" $ do
            ...
            let res = simulate "move.l 12, D0" st0
             in (dr res !? D0) @?= Just 12
        , testCase "Read byte from memory by address register" $ do
            let State{dr, ar} = simulate "move.b (A2), D0" st0{ar = insert A2 6 ar0}
             in do
                    (dr !? D0) @?= Just 6
                    (ar !? A2) @?= Just 6
            let State{dr, ar} =
                    simulate
                        "move.b (A2), D0"
                        st0{ar = insert A2 6 ar0, dr = insert D0 0x10203040 dr0}
             in do
                    (dr !? D0) @?= Just 0x10203006
                    (ar !? A2) @?= Just 6
            let State{dr, ar} = simulate "move.b -(A2), D0" st0{ar = insert A2 6 ar0}
             in do
                    (dr !? D0) @?= Just 5
        ...
        , testCase "Write byte from memory by address register" $ do
            let State{dr, ar, mem} = simulate "move.b D2, (A2)" st0{ar = insert A2 6 ar0}
             in do
                    (dr !? D2) @?= Just 2
                    (ar !? A2) @?= Just 6
            ...
            let State{dr, ar, mem} = simulate "move.b D2, (A2)" st0{ar = insert A2 6 ar0, dr = insert D2 0x10203040 dr0}
             in do
                    (dr !? D2) @?= Just 0x10203040
                    (ar !? A2) @?= Just 6
        ...
        , testCase "Read word from memory by address register" $ do
            let State{dr, ar,mem} = simulate "move.l (A2), D0" st0{ar = insert A2 6 ar0}
             in do
                    (dr !? D0) @?= Just 0x09080706
                    (ar !? A2) @?= Just 6
                    readMemBytes mem [5, 6, 7, 8, 9, 10] @?= [5, 0x06, 0x07, 0x08, 0x09, 10]
            let State{dr, ar, mem} = simulate "move.l -(A2), D0" st0{ar = insert A2 6 ar0}
             in do
                    (dr !? D0) @?= Just 0x05040302
                    (ar !? A2) @?= Just 2
        ...
        , testCase "Write word to memory by address register" $ do
            let State{dr, ar, mem} = simulate "move.l D2, (A2)" st0{ar = insert A2 6 ar0}
             in do
                    (dr !? D2) @?= Just 2
                    (ar !? A2) @?= Just 6
                    readMemBytes mem [5, 6, 7, 8, 9, 10] @?= [5, 0x02, 0x00, 0x00, 0x00, 10]
            let State{dr, ar, mem} = simulate "move.l D2, (A2)" st0{ar = insert A2 6 ar0, dr = insert D2 0x10203040 dr0}
             in do
                    (dr !? D2) @?= Just 0x10203040
                    (ar !? A2) @?= Just 6
    ...
    where
        ...
        st0 :: M68kState Int32
        st0@State{ar = ar0, dr = dr0} =
            (initState 32 $ mkIoMem (fromList []) mem0)
                { dr = fromList $ zip dataRegisters [0 ..]
                }
