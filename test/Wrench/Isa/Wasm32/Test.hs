...
tests =
    testGroup
        ...
        , testCase "Source metadata lowers to an embedded FuncHeader" $ do
            ...
            case translateWasm32 @Int32 64 "-" src of
                ...
                Right (TranslatorResult dump labels _stats, functions) -> do
                    ...
                    prettyDump labels (dumpCells dump)
                        @?= "mem[0..3]: \tFuncHeader 0 0 0 \t@_start\nmem[4..4]: \tHalt\nmem[5..5]: \tReturn\nmem[6..63]: \t( 00 )"
...
-- with a 'FuncHeader' occupying its first 4 bytes; `call` reads it
