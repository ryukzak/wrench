...
- **Divide Step**
    - **Syntax:** `+/`
    - **Description:** Perform one restoring-division step. The **dividend** is in register `A`; the **divisor** is the word at `mem[B]`. The partial **remainder** lives in `S`, and the partial **quotient** in `T`. After 32 calls the final quotient is in `T`, the remainder in `S`.
    - **Operation:** `S <- S << 1; if A[31] then S[0] <- 1; A <- A << 1; if S >= mem[B] then S <- S - mem[B]; T <- T << 1; if S >= mem[B] then T[0] <- 1`
