...
- **Divide Step**
    ...
    - **Operation:** `remainder <- remainder << 1; if dividend[31] then remainder[0] <- 1; dividend <- dividend << 1; if remainder >= divisor then remainder <- remainder - divisor; quotient <- quotient << 1; if remainder >= divisor then quotient[0] <- 1`, где `A <- dividend; [B] <- divisor; T -> quotient; S -> remainder`
