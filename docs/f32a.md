...
- **B-Fetch**
    - **Syntax:** `b`
    - **Description:** Push the value of register B onto the data stack.
    - **Operation:** `dataStack.push(B)`
...
- **Over**
    - **Syntax:** `over`
    - **Description:** Copy the second value on the data stack to the top, pushing the data stack.
    - **Operation:** `T <- dataStack.pop(); S <- dataStack.pop(); dataStack.push(S); dataStack.push(T); dataStack.push(S)`
