...
- One shared stack, holding locals, operand values, and structured-control/call bookkeeping alike -- no general-purpose registers
...
There is exactly one stack, and it holds three different kinds of things at once: a function's locals, its operand values, and bookkeeping records for open `block`/`loop`/`if`/call scopes. Nothing about the instruction encoding distinguishes them -- a local is just a word at a fixed address, an operand is just a word `sp` currently points at, and a control record is just a handful of words that happen to describe a scope. Reading the raw stack (as the `stack` state view does) shows all of it, interleaved in whatever order it was actually pushed. Distinguishing the two contents-wise, the plain pushed values are the *operand stack*; the block/loop/call bookkeeping records are the *control stack* -- both live on the one shared stack described here, not on separate regions of their own.

Memory is split in half: the lower half holds code and `.data`; the upper half is where the stack lives. The stack descends: it starts at the very top of memory and grows toward lower addresses as things are pushed, the classic hardware convention -- `sp` (the stack pointer) is the address of the most recently pushed word, decremented *before* a push writes and incremented *after* a pop reads, and only ever moves that way.

### Locals

A function's locals are exactly its parameters -- there is no instruction to declare more. They occupy a contiguous run of words starting at `frameBase`, one per index, addressed directly (`frameBase - index * 4` -- the stack descends, so local `0`, the first argument pushed, ends up at the *highest* address and each later one lower): whatever values the caller pushed just before `call` are, from the callee's perspective, its locals `0` through `paramCount - 1`. `frameBase` is not fixed; it moves to wherever the current function's locals happen to start, and is saved and restored across calls (see Functions, below).
...
`block` and `loop` push a bookkeeping record onto the very same stack values live on, the moment they are entered -- not somewhere separate. That record physically sits between whatever was pushed *before* the scope opened and whatever gets pushed *inside* it. An instruction that needs an operand only ever reaches from the current top of the stack; it has no way to reach past a record sitting in the way to a value that was pushed earlier.
...
Each open `block`, `loop`, or call is a record written directly into the stack at the moment it is entered: a link back to whatever record was innermost before it (packed as an offset alongside its own tag, rather than a full word to itself, since it only ever points backward by however far this scope's own body has pushed), and whatever else its own kind needs -- a `block` just its own exit address; a `loop` its re-entry address and its exit address, the latter stored as an offset from the former since both sit in the same loop body; a call the caller's saved `frameBase`, its saved local count, its declared result count, and its own return address. `block` is a single word; `loop` and a call are both 2. The most recently opened record's address is tracked separately (`ctrlTop`); every other one is only reachable by following the chain of links outward from it.

`Link` itself is one-directional -- it only ever points backward, to the strictly-higher address of whatever record was innermost before this one (the stack descends, so whatever was pushed earlier sits above whatever's pushed now) -- so it's stored as `LinkOffset = Link - RecordAddr` (a *byte* distance) instead of an absolute address. It's unsigned, since a record only ever points backward; `0` -- otherwise unreachable, since the previous record always has some nonzero width of its own -- is the "no link" sentinel, replacing the signed `-1` a bare absolute address needed.

Every address-shaped field here -- `CsStart`/`CsEnd` (code addresses, inside `.text`), `ReturnPc` (also code), `SavedFrameBase` (a stack address) -- gets the same uniform 16 bits: there's no ISA-level reason to trust one part of the address space more than another, only today's empirically small example programs, and 16 bits (up to 65535) is enormous headroom either way. `LinkOffset` gets 14 bits: packed into the same word as `Tag` (2 bits), that's exactly half a word, comfortably covering the configured memory even well past the default memory-size limit.
...
```erlang
%% block -- 4 bytes
<<Tag:2, LinkOffset:14, CsEnd:16>>

%% loop -- 8 bytes
<<Tag:2, LinkOffset:14, CsStart:16,       % word0
  CsEndOffset:16, 0:16>>                  % word1 -- CsEnd = CsStart + CsEndOffset

%% call -- 8 bytes
<<Tag:2, LinkOffset:14, SavedLocalCount:8, ResultCount:8,  % word0
  ReturnPc:16, SavedFrameBase:16>>                          % word1
```

`block` is a single word; `loop` and `call` are both 2. The only unused bits are `loop`'s own second word's low 16, since `CsEndOffset` alone already fits everything a loop's own exit needs.

`Tag` only ever needs 2 bits (3 possible values); `SavedLocalCount`/`ResultCount` get a full byte each (0-255, generous for a real program's param/result count).

A `br`/`br_if` naming depth `n` walks that chain outward `n` steps from the innermost record to find its target, then closes every record strictly between the branch and the target (regardless of that record's own kind), and finally either re-enters the target (if it is a loop) or closes it too (if it is a block). Closing a `block`/`loop` record removes exactly its own words from the stack, sliding whatever was pushed below it (more recently, since the stack descends) up to take their place -- so anything a scope's body pushed and never popped survives past the scope closing, right where it would have been if the record had never been there.
...
At the moment `call` executes, the values pushed immediately before the popped target -- as many as its declared argument count -- become the callee's locals `0` downward (the stack descends, so the first argument pushed ends up at the highest address), without being copied anywhere: `call` simply records where they already are as the new `frameBase` and jumps to the target. A record describing the call (the caller's own `frameBase`, its own local count, the return address, and the declared result count) is pushed immediately below those arguments, on the same stack every other record and every operand lives on.
...
- `layout:dec`, `layout:hex` -- an annotated dump of the whole live stack, push-order (highest/oldest address first) throughout, from the outermost active call down to the innermost/live one. Every *suspended* frame (one that made a call and is waiting on it) gets a `#N <function> (pc=<pc>)` header -- both genuinely backed by memory, since that `pc` is exactly the `csReturnPc` sitting in the `CallScope` that suspended it (the function name is just the nearest label at or before that address). The innermost/live frame -- the one actually running -- gets no header at all: neither its function name nor its `pc` are stored anywhere (they live only in the interpreter's own state), and the live `pc` is already shown separately by the `pc`/`pc:label` views, so repeating it here would just be the same non-memory value shown twice. Every frame's own spans follow the same shape regardless: locals and genuine operand values each render as a `mem[a..b]: locals`/`mem[a..b]: (operands)` header with one indented `index: value` line per word (index `0` = the highest address, i.e. the first param or the first value pushed -- push order); if the innermost frame's own operand stack is currently empty, that's shown explicitly as `(operands): empty` rather than silently omitted (an *interior* gap between two of a frame's own control records being empty stays silent, though -- unremarkable, unlike nothing at all being on top right now). Each of a frame's own open control records renders as one line, its `mem[a..b]` range followed directly by a Haskell-record-literal rendering of its fields (e.g. `mem[0x108..0x10f]: CallScope { csSavedFrameBase = 0x100, csSavedLocalCount = 0, csReturnPc = 0x037, csResultCount = 1, linkOffset = none }`) rather than the record's own raw words, which aren't independently readable once packed (see "Control records, precisely" above) and would just repeat what the decoded fields already say -- the same information `stack`/`locals` give, but with every word labeled as a real value or a piece of control-record bookkeeping instead of left looking like undifferentiated data. The trailing `linkOffset` isn't a `Scope` field -- `link` is stored alongside the record, not in it -- but the exact same byte distance `LinkOffset` packs into the record's own first word (see "Control records, precisely" above), or `none` for the outermost record (`link` was `nullAddr`). `:hex` formats every *address* this view mentions the same way -- `mem[a..b]` ranges, a frame's own `pc=`, and a record's own `csStart`/`csEnd`/`csReturnPc`/`csSavedFrameBase` fields -- not just the plain word values, and only as many hex digits as this run's own configured memory could ever need (a 512-byte memory needs 3, not `word32ToHex`'s usual 8, which stays for arbitrary-width word *values*); a record's own counts (`csResultCount`, `csSavedLocalCount`) and `linkOffset` stay decimal in both formats, since hex doesn't make a count or a byte distance more readable.
...
- **Initialize Stack Pointer**
    - **Syntax:** `sp.init`
    - **Description:** Pop an address and relocate the stack to start there -- both `sp` and `frameBase` move to it. Only meaningful before anything has been pushed (typically the first thing `_start` does). A push decrements before writing, so the given address itself is never written to -- leave room *below* it (lower addresses) for the deepest this run's stack will ever get. Without this, the stack always starts at the very top of memory, which a program with enough `.data` to spill past the halfway point has no way to override. Nothing checks that the new address avoids `.text`/`.data`.
