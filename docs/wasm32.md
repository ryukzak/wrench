...
`Link` itself is one-directional -- it only ever points backward, to the strictly-lower address of whatever record was innermost before this one -- so it's stored as `LinkOffset = RecordAddr - Link` (a *byte* distance) instead of an absolute address. It's unsigned, since a record only ever points backward; `0` -- otherwise unreachable, since the previous record always has some nonzero width of its own -- is the "no link" sentinel, replacing the signed `-1` a bare absolute address needed.
