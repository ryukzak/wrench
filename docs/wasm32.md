...
- `layout:dec`, `layout:hex` -- an annotated dump of the whole live stack, address-ascending, from the outermost active call down to the innermost/live one. Each frame gets a `#N <function> (pc=<pc>)` header (the function name is the nearest label at or before that frame's own pc), followed by one span per contiguous piece of its own visible range: its locals and its genuine operand values each render as a `mem[a..b]: locals`/`mem[a..b]: (stack)` header with one indented `index: value` line per word (index `0` = the lowest address, i.e. the first param or the first value pushed), and each of its own open control records renders as a raw-words line tagged `@loop`/`@block`/`@call` followed by that record's own fields one per line (`return=..`, `results=..`, `savedFrameBase=..`, `savedLocalCount=..` for a call; `start=..`/`end=..` for a loop; `end=..` for a block) -- the same information `stack`/`locals` give, but with every word labeled as a real value or a piece of control-record bookkeeping instead of left looking like undifferentiated data. `:hex` formats every *address* this view mentions the same way -- `mem[a..b]` ranges, a frame's own `pc=`, and a call/loop/block's `return`/`start`/`end`/`savedFrameBase` fields -- not just the plain word values; a record's own counts (`results`, `savedLocalCount`) stay decimal in both formats, since hex doesn't make a count more readable.
