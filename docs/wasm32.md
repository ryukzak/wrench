...
Each open `block`, `loop`, or call is a record written directly into the stack at the moment it is entered: a link back to whatever record was innermost before it, a "meta" word packing its tag together with its two small counts (a call's saved local count and declared result count -- 0 for a `block`/`loop`), and one or two more words for whichever address-shaped fields that kind actually needs (a `block` needs only its own exit address; a `loop` needs its re-entry address too; a call needs the caller's saved `frameBase` and its own return address). A `block`'s record is 3 words; a `loop`'s or a call's is 4 -- width follows the kind, not one size fixed for all of them. The most recently opened record's address is tracked separately; every other one is only reachable by following the chain of links outward from it.
...
- `layout:dec`, `layout:hex` -- an annotated dump of the whole live stack, address-ascending, from the outermost active call down to the innermost/live one. Each frame gets a `#N <function> (pc=<pc>)` header (the function name is the nearest label at or before that frame's own pc), followed by one span per contiguous piece of its own visible range: its locals and its genuine operand values each render as a `mem[a..b]: locals`/`mem[a..b]: (stack)` header with one indented `index: value` line per word (index `0` = the lowest address, i.e. the first param or the first value pushed), and each of its own open control records renders as one line, its `mem[a..b]` range followed directly by a Haskell-record-literal rendering of its fields (e.g. `mem[0x108..0x117]: CallScope { csSavedFrameBase = 0x100, csSavedLocalCount = 0, csReturnPc = 0x037, csResultCount = 1 }`) rather than the record's own raw words, which aren't independently readable once packed (see "Control records, precisely" above) and would just repeat what the decoded fields already say -- the same information `stack`/`locals` give, but with every word labeled as a real value or a piece of control-record bookkeeping instead of left looking like undifferentiated data. `:hex` formats every *address* this view mentions the same way -- `mem[a..b]` ranges, a frame's own `pc=`, and a record's own `csStart`/`csEnd`/`csReturnPc`/`csSavedFrameBase` fields -- not just the plain word values, and only as many hex digits as this run's own configured memory could ever need (a 512-byte memory needs 3, not `word32ToHex`'s usual 8, which stays for arbitrary-width word *values*); a record's own counts (`csResultCount`, `csSavedLocalCount`) stay decimal in both formats, since hex doesn't make a count more readable.
