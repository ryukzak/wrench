...
endFunc :: Parser (Wasm32Isa w (Ref w))
endFunc = try $ do
    optionalDot
    void $ string "endfunc"
    return EndFunc

numericFunc :: Parser (Wasm32Isa w (Ref w))
numericFunc = do
    params <- number
    comma
    locals <- number
...
keywordFunc :: Parser (Wasm32Isa w (Ref w))
keywordFunc = buildFunc <$> some funcWord

buildFunc :: [String] -> Wasm32Isa w l
buildFunc tokens =
...
lookupLocal :: (IsWord w) => String -> State (Wasm32St w) w
lookupLocal name = do
    currentFrame >>= \case
        Nothing -> do
...
setLocal :: String -> w -> State (Wasm32St w) ()
...
enterFunction :: (IsWord w) => Wasm32Isa w w -> State (Wasm32St w) ()
enterFunction instruction@Func{funcParams, funcLocals, funcResults} = do
    st@Wasm32St{frames, pendingCall} <- get
    case pendingCall of
